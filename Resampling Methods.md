...
* for example, these methods can be used to estimate the test set #prediction-error-estimates 
...
* the #test-error is the #mean error that results from using a #statistical-learning method to predict the response on a new observation, one that was not used in training the method
* the #training-error is the #mean error that results from using a #statistical-learning method to predict the response on the same observations that were used in training the method
* the #training-error rate often is quite different from the [[Overview of Statistical Learning#2.6 Bias-variance trade-off|test error]], and in particular the former can dramatically underestimate the latter
...
    * these include the #Mallows-Cp, #aic, #bic, and #adjusted-R2 statistics
...
* the resulting #validation-error rate, typically assessed using #mse in the case of a quantitative response, or #misclassification-rate in the case of a qualitative response, provides an estimate of the #test-error rate
## 5.4.1 Drawbacks of #validation-set approach
* the #validation-error rate can be highly variable, depending on precisely which observations are included in the #training-set and which observations are included in the #validation-set
...
    * this suggests that the #validation-error rate may tend to *overestimate* the #test-error rate for the model fit on the entire data set
    
...
* with #least-squares #linear-regression or #polynomial-regression, an amazing shortcut makes the cost of #LOOCV the same as that of a single model fit!
...
    * fit a #logistic-regression[^1] model using the 100 features
### the wrong way
